...
async def lifespan(app: FastAPI):
    ...
    # Initialize Emergency Green Corridor System (FRD-07)
    ...
    from app.api.emergency_routes import set_emergency_components
    from app.services import get_map_loader_service
    
    # Get map loader for junction/road data
    map_loader = get_map_loader_service()
    
    # Initialize emergency components
    emergency_tracker = init_emergency_tracker(
        map_loader=map_loader,
        ws_emitter=ws_emitter
    )
    
    emergency_pathfinder = init_emergency_pathfinder(map_loader=map_loader)
    
    emergency_corridor_manager = init_corridor_manager(
        mode_manager=mode_manager,
        emergency_tracker=emergency_tracker,
        pathfinder=emergency_pathfinder,
        map_loader=map_loader,
        ws_emitter=ws_emitter
    )
    
    # Set components for API routes
    set_emergency_components(
        tracker=emergency_tracker,
        pathfinder=emergency_pathfinder,
        corridor_manager=emergency_corridor_manager
    )
    
    print("[OK] Emergency Green Corridor initialized (FRD-07)")
    
    # Initialize Post-Incident Tracking System (FRD-08)
    ...
    from app.api.incident_routes import set_incident_components
    
    # Initialize detection logger
    detection_logger = init_detection_logger({
        'bufferSize': 100,
        'flushInterval': 5.0,
        'retentionHours': 24
    })
    await detection_logger.start()
    
    # Initialize inference engine with map service
    inference_engine = init_inference_engine(
        map_service=map_loader,
        config={
            'avgCitySpeed': 30.0,
            'maxSearchRadius': 10.0,
            'detectionTimeWindow': 3600
        }
    )
    
    # Initialize incident manager
    incident_manager = init_incident_manager(
        inference_engine=inference_engine,
        ws_emitter=ws_emitter
    )
    
    # Set components for API routes
    set_incident_components(
        incident_manager=incident_manager,
        inference_engine=inference_engine,
        detection_logger=detection_logger
    )
    
    print("[OK] Post-Incident Tracking System initialized (FRD-08)")
    
    # Initialize Auto-Challan System (FRD-09)
    ...
    from app.api.challan_routes import set_challan_components
    
    # Load challan configuration
    challan_config = {}
    try:
        import json
        with open('config/challan.json', 'r') as f:
            challan_config = json.load(f)
    except Exception as e:
        print(f"[INFO] Using default challan config: {e}")
    
    # Initialize challan components
    violation_detector = init_violation_detector(challan_config)
    vehicle_owner_db = init_vehicle_owner_db(challan_config.get('mockOwners', {}))
    challan_manager = init_challan_manager(vehicle_owner_db)
    auto_challan_service = init_auto_challan_service(
        violation_detector=violation_detector,
        challan_manager=challan_manager,
        config=challan_config
    )
    
    # Set WebSocket emitter for real-time notifications
    auto_challan_service.set_ws_emitter(ws_emitter)
    
    # Set components for API routes
    set_challan_components(
        violation_detector=violation_detector,
        vehicle_owner_db=vehicle_owner_db,
        challan_manager=challan_manager,
        auto_challan_service=auto_challan_service
    )
    
    # Start auto-challan service
    await auto_challan_service.start()
    
    print("[OK] Auto-Challan System initialized (FRD-09)")
    ...
    # Stop detection logger (FRD-08)
    try:
        from app.incident import get_detection_logger
        det_logger = get_detection_logger()
        if det_logger:
            await det_logger.stop()
            print("[SHUTDOWN] Detection logger stopped")
    except Exception as e:
        print(f"[SHUTDOWN] Error stopping detection logger: {e}")
    
    # Stop auto-challan service
    try:
        from app.challan import get_auto_challan_service
        challan_svc = get_auto_challan_service()
        if challan_svc:
            await challan_svc.stop()
            print("[SHUTDOWN] Auto-challan service stopped")
    except Exception as e:
        print(f"[SHUTDOWN] Error stopping auto-challan service: {e}")
